...
Client.prototype.submitJob = function(name, payload, options) {
    ...
    var jsSendCallback = function(err) {
        if (err instanceof Error) {
            Client.logger.log('warn', 'failed to submit job, server=%s, error=%s', jobServer.getUid(), err.stack);
            tryToSend();
        } else {
            ...
            Client.logger.log('debug', 'job submited, name=%s, unique=%s, jobServerUid=%s', job.name, job.unique, job.jobServerUid);
        }
    }
