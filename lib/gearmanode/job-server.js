    ...
    if (this.connected) {
        ...
        eventNames.forEach(function (name) {
            self.socket.on(name, function() {
                JobServer.logger.log('debug', 'unhandled event, name=%s', name);
            });
        });
...
JobServer.prototype.echo = function (data) {
    ...
    this.send(packet);
    JobServer.logger.log('debug','echo back REQ, data=%s', data);
}
...
JobServer.prototype._processData = function (chunk) {
    ...
    case protocol.PACKET_TYPES.ERROR:
        JobServer.logger.log('error', 'job server error, uid=%s, code=%s, msg=%s',
